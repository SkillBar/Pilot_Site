...
export function PilotInfrastructureSection() {
  ...
  useEffect(() => {
    ...
    const syncMedia = () => {
      setMediaActive(
        isNearViewport && desktopLayout.matches && !reducedMotion.matches,
      );
    };
    ...
    desktopLayout.addEventListener("change", syncMedia);

    return () => {
      observer.disconnect();
      reducedMotion.removeEventListener("change", syncMedia);
      desktopLayout.removeEventListener("change", syncMedia);
    };
  }, []);
